...
def reset_config(request_path, respond_path):
    """
    读取 as_request.json 的请求参数，准备 as_respond.json

    Args:
        request_path: as_request.json 文件路径
        respond_path: as_respond.json 文件路径
    """
    ...
    try:
        ...
        print(f"读取请求配置文件失败: {e}")
        return False, None

    # 创建响应配置结构
    respond_config = {
        'request': {
            'server_url': request_config.get('server_url', ''),
            'c_sn': request_config.get('c_sn', ''),
            'u_sn': request_config.get('u_sn', ''),
            'for_organization': request_config.get('for_organization', ''),
            'u_url': request_config.get('u_url', '')
        },
        'response': {}
    }

    print(f"配置文件读取成功")
    return True, respond_config
...
def main(g_camera_id=None):
    """主函数，执行完整的设备注册流程

    Args:
        g_camera_id: 可选，从 NVS 中读取的全局相机 ID，用于设置 c_sensor 参数
    """
    ...
    if not os.path.exists(REQUEST_CONFIG_PATH):
        print(f"未找到请求配置文件: {REQUEST_CONFIG_PATH}")
        print("请在 as_dm_register 目录下创建 as_request.json")
        return create_error_result("未找到请求配置文件")

    # 读取请求配置并准备响应配置
    success, respond_config = reset_config(REQUEST_CONFIG_PATH, RESPOND_CONFIG_PATH)
    if not success:
        return create_error_result("配置文件读取失败")

    # 提取请求参数
    unit_config = respond_config['request']
    ...
    if g_camera_id:
        ...
        print(f"添加 c_sensor 参数: {g_camera_id}")

    print(f"相机数据: {camera_data}")
    ...
    # 更新响应配置
    respond_config['response']['u_camera_id'] = u_camera_id
    respond_config['response']['u_unit_id'] = u_unit_id
    respond_config['response']['u_account_id'] = u_account_id
    respond_config['response']['password'] = password
    respond_config['response']['device_token'] = device_token

    # 写入响应配置文件
    try:
        with open(RESPOND_CONFIG_PATH, 'w', encoding='utf-8') as file:
            json.dump(respond_config, file, indent=4, ensure_ascii=False)
        print(f"响应配置文件保存成功: {RESPOND_CONFIG_PATH}")
    except Exception as e:
        print(f"保存响应配置文件失败: {e}")
        return create_error_result(f"保存响应配置文件失败: {e}")
    ...
    print("注册数据已保存到 as_respond.json")
